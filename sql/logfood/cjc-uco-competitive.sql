...
competitive_ucos AS (
  ...
  WHERE uco.snapshot_date = (SELECT MAX(snapshot_date) FROM main.gtm_silver.use_case_detail)
    ...
    AND uco.competitors IS NOT NULL
    AND uco.competitors != 'No Competitor'
)
SELECT
  primary_competitor,
  competitor_category,
  ...
  SUM(CASE WHEN stage = 'U3' THEN 1 ELSE 0 END) AS scoping,
  SUM(CASE WHEN stage IN ('U4', 'U5') THEN 1 ELSE 0 END) AS near_win,
  SUM(CASE WHEN stage = 'U6' THEN 1 ELSE 0 END) AS live,
...
GROUP BY primary_competitor, competitor_category
ORDER BY
