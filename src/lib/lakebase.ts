...
function generateToken(): string {
  try {
    ...
    throw new Error(`Failed to generate Lakebase credential: ${(error as Error).message}`)
  }
}
...
function getCurrentUser(): string {
  try {
    ...
    throw new Error(`Failed to get Databricks user: ${(error as Error).message}`)
  }
}
...
export async function fetchStats(): Promise<LakebaseStats> {
  ...
  return {
    totalItems: Number.parseInt(row.total_items),
...
export async function addTestItem(item: {
  ...
  const result = await pool.query<TestItem>(
    `INSERT INTO app.test_items (name, description, price, quantity)
     VALUES ($1, $2, $3, $4)
     RETURNING *`,
    [item.name, item.description || null, item.price || null, item.quantity || 0]
  )
